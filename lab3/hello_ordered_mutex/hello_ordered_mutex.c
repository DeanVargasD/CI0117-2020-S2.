...
void* helloWorld(void* args) {
    ...
    pthread_mutex_lock(&shared_data->mutex_position[thread_num]);
    ...
    pthread_mutex_unlock(&shared_data->mutex_position[thread_num+1]);
...
int main(int argc, char* arg[]) {
    ...
    shared_data->mutex_position = (pthread_mutex_t*)calloc(thread_count, sizeof(pthread_mutex_t));

    shared_data->total = thread_count;
    shared_data->position = 1;

    for(size_t i = 0; i<thread_count; ++i){
    pthread_mutex_init(&shared_data->mutex_position[i], NULL);
    }  

    for(size_t i = 1; i<thread_count; ++i){
    pthread_mutex_lock(&shared_data->mutex_position[i]);
    }
    ...
    for (size_t i = 0; i < thread_count; ++i) {
     ...
     for (size_t i = 0; i < thread_count; ++i) {
        pthread_mutex_destroy(&shared_data->mutex_position[i]);
    }
    ...
    free(threads);
    free(shared_data->mutex_position);
    free(shared_data);
